...
Preferences are the only durable application state. `sanitizeSettings()` always starts from defaults, selectively accepts known values, deep-copies nested objects, and drops unknown keys. The current schema version is 11.

| Field | Default | Accepted/sanitized value | Persisted | In Scene |
| --- | --- | --- | --- | --- |
| `schemaVersion` | `11` | Always current default; input is not copied | Yes | No |
...
Before sanitization, a loaded document with `schemaVersion < 3` receives `borderWidth = 0` and `size = 288`. There are no explicit migrations for schema 3–10; defaults and sanitization provide forward compatibility.
