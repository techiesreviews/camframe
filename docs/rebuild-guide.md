...
This guide is for reconstructing the current source behavior based on v0.4.3 plus the unreleased onboarding change, not redesigning it. Read `known-gaps.md` first so dormant or aspirational code is not mistaken for live behavior.

## Reproducible baseline

- Git commit/tag: `fe8e6c6` / `v0.4.3`
- Working-tree extension: versioned contextual Overlay onboarding from ADR 0012 and ADR 0016–0023; Preferences schema 12
...
1. **Pure domain contracts** — implement `settings.js`, `cameras.js`, `fullscreen.js`, and `onboarding.js` first. Reproduce defaults, clamps, geometry, Scene merge/order, constraints, onboarding migration/copy/steps, and 280 ms interpolation. Port the pure tests before adding Electron.
...
8. **Full screen** — animate native bounds rather than toggling Electron fullscreen; coordinate renderer geometry, stream profile, Escape, toolbar hotspot, and state restoration.
9. **Scenes** — preserve the persisted `presets` name and import compatibility, cap/order semantics, live position snapshot, compact animation, tray, shortcuts, and notices.
...
11. **Packaging** — restrict ASAR inputs, embed only current fonts/icons and English locale, add macOS camera usage copy, and reproduce artifact names.
12. **Dormant Controller** — preserve only if exact source parity is required. Do not instantiate it for behavioral parity with the current source.

## Reconstruction invariants

- One camera MediaStream per Overlay; no canvas frame-copy pipeline.
- Full screen/profile/Scene changes reuse the stream unless camera selection itself changes.
...
- “Controller” names in IPC do not imply the separate Controller is live.
...
2. Complete every applicable row in the manual Windows matrix, including multiple-camera/Elgato tests for profile switching.
...
| Separate Controller | `src/control.*`, `src/main.js` | dormant-surface decision and parity with Inline settings |
