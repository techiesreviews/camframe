...
This guide is for reconstructing the checked-in v0.4.3 behavior, not redesigning it. Read `known-gaps.md` first so dormant or aspirational code is not mistaken for live behavior.
...
1. **Pure domain contracts** — implement `settings.js`, `cameras.js`, and `fullscreen.js` first. Reproduce defaults, clamps, geometry, Scene merge/order, constraints, and 280 ms interpolation. Port the pure tests before adding Electron.
...
10. **Packaging** — restrict ASAR inputs, embed only current fonts/icons and English locale, add macOS camera usage copy, and reproduce artifact names.
11. **Dormant Controller** — preserve only if exact source parity is required. Do not instantiate it for behavioral parity with v0.4.3.
